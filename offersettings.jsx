...
const LS_KEY = "vt.offerSettings.check_free_offer";
const PAGE_TABS = ["Project matchings", "Offer settings", "Business indicators", "Technical indicators"];
...
const CATS = [
  ...
  { group: "Negotiation", items: [
    { id: "negotiation", name: "Acceptance & agent", icon: "triggers" },
  ]},
];
    ...
    return (
      <div className="osf-full">
        <div className="osf-field"><span className="osf-name">{field.label}</span><span className="osf-info" title={field.meaning}><Icon name="info" size={13} /></span>
          {canNegotiate && <button type="button" className={`osf-negbtn ${negotiable ? "on" : ""}`} style={{ marginLeft: "auto" }} onClick={() => set((s) => { s.neg[field.id] = !s.neg[field.id]; })} aria-pressed={negotiable}><span className="mini-toggle"><i /></span>Negotiable</button>}
        </div>
    ...
    <div className="osf-row">
      <div className="osf-line">
        ...
        <div className="osf-controls">
          ...
          {canNegotiate && <button type="button" className={`osf-negbtn ${negotiable ? "on" : ""}`} onClick={() => set((s) => { s.neg[field.id] = !s.neg[field.id]; })} aria-pressed={negotiable}><span className="mini-toggle"><i /></span>Negotiable</button>}
        </div>
        {hasDefault && (matches
...
// Reusable negotiation strip for non-schema fields (pricing amounts, usage policies).
...
function MiniNegStrip({ autoAgent, acceptEl, note, onNote }) {
  ...
  return (
    <div className="osf-strip" style={{ marginTop: 10, marginBottom: 0 }}>
      ...
      {noteOpen && <textarea className="os-ta" placeholder="What you will and won't accept for this term…" value={note} onChange={(e) => onNote(e.target.value)} />}
    </div>
  );
}
...
function NegStrip({ field, st, set, noteOpen, setNoteOpen }) {
  return (
    <div className="osf-strip">
      ...
      {noteOpen && <textarea className="os-ta" placeholder="What you will and won't accept for this term…" value={st.notes[field.id]} onChange={(e) => set((s) => { s.notes[field.id] = e.target.value; })} />}
    </div>
  );
}
  ...
  return (
    <div className={`trow ${open ? "open" : ""}`}>
      <button type="button" className="trow-head" onClick={() => setOpen((o) => !o)} aria-expanded={open}>
        ...
        <span className={`trow-state ${canNegotiate && negotiable ? "neg" : "fixed"}`}>{canNegotiate && negotiable ? <><Icon name="triggers" size={10} /> Negotiable</> : "Fixed"}</span>
        ...
        <div className="trow-body">
          ...
          {canNegotiate && (
            <div className="trow-negrow">
              <button type="button" className={`osf-negbtn ${negotiable ? "on" : ""}`} onClick={() => set((s) => { s.neg[field.id] = !s.neg[field.id]; })} aria-pressed={negotiable}><span className="mini-toggle"><i /></span>Negotiable</button>
              <span className="trow-neghint">{negotiable ? "The contract agent may settle this term automatically, within the range below." : "Fixed — every taker gets exactly this value; not open to negotiation."}</span>
            </div>
          )}
...
function TermsPanel({ section, st, set, onAdopt }) {
  const intro = (
    <>
      <DataspaceBaselineCard section={section} st={st} set={set} onAdopt={onAdopt} />
      <AiFillBar section={section} st={st} set={set} />
      {!section.repeatable && (
        <ActionExplain icon="triggers" title={'“Negotiable” vs “Fixed” terms'}>
          Mark a term <b>Negotiable</b> to let the contract agent settle it automatically, within a range you choose — deals close without waiting on you. Terms left <b>Fixed</b> apply to every taker exactly as published and are never up for discussion.
        </ActionExplain>
      )}
    </>
  );
  ...
  if (section.groups) {
    return <>{intro}{section.groups.map((g) => (<div key={g.label}><div className="os-group-label">{g.label}</div>{g.fields.map((f) => <TermRow key={f.id} field={f} st={st} set={set} />)}</div>))}</>;
  }
  return <>{intro}{section.fields.map((f) => <TermRow key={f.id} field={f} st={st} set={set} />)}</>;
}
...
// ─── PRICING ──────────────────────────────────────────────────────────────────
// A monetary pricing field: value input + a Negotiable toggle, mirroring T&C fields.
function PriceField({ k, label, req, help, st, set }) {
  const p = st.pricing;
  const neg = !!st.pricingNeg[k];
  const acc = st.pricingAccept[k] || {};
  return (
    <div className={neg ? "full" : undefined}>
      <label className="os-flabel">{label}{req && <em>*</em>}</label>
      {help && <div className="os-fhelp">{help}</div>}
      <input className="os-in" value={p[k]} onChange={(e) => set((s) => { s.pricing[k] = e.target.value; })} />
      <div className="os-price-neg">
        <button type="button" className={`osf-negbtn ${neg ? "on" : ""}`} onClick={() => set((s) => { s.pricingNeg[k] = !s.pricingNeg[k]; })} aria-pressed={neg}><span className="mini-toggle"><i /></span>Negotiable</button>
      </div>
      {neg && <MiniNegStrip autoAgent={st.autoAgent}
        acceptEl={<span className="os-range"><Num value={acc.min} onChange={(min) => set((s) => { s.pricingAccept[k] = { ...s.pricingAccept[k], min }; })} /><span className="dash">to</span><Num value={acc.max} onChange={(max) => set((s) => { s.pricingAccept[k] = { ...s.pricingAccept[k], max }; })} /><span className="os-unit">{p.currency}</span></span>}
        note={st.pricingNotes[k]} onNote={(v) => set((s) => { s.pricingNotes[k] = v; })} />}
    </div>
  );
}
function PricingPanel({ st, set }) {
  ...
  return (
    <>
      ...
      <div className="os-note"><Icon name="info" size={15} /> <span>If pricing is not applicable, set the numeric fields to <b>0</b>. Turn on <b>Negotiable</b> to let the contract agent settle an amount within a range you define.</span></div>
...
function NegotiationPanel({ st, set }) {
  const negTotal = ALL_FIELDS.filter((f) => st.neg[f.id] && f.type !== "date").length;
  const fixedTotal = ALL_FIELDS.filter((f) => !st.neg[f.id] && f.type !== "date").length;
  return (
    <>
      <ActionExplain icon="ai" title="Why let the contract agent negotiate?">
        Takers rarely subscribe at your exact published terms. Rather than emailing back and forth, the contract agent settles negotiable terms on your behalf — automatically accepting any request that lands inside the acceptance range you set per term. Deals close around the clock, and only requests that fall outside your limits are escalated to you.
      </ActionExplain>
      <div className="os-neg-stats">
        <div className="os-stat neg"><b>{negTotal}</b><span>Negotiable terms</span></div>
      ...
      <div className="os-ncrow">
        <div><div className="nc-title">Taker accepts your baseline terms (no negotiation)</div><div className="nc-desc">What happens when a user subscribes at exactly the terms you published.</div></div>
      ...
      <div className="os-ncrow">
        <div><div className="nc-title">Contract agent auto-negotiation</div><div className="nc-desc">Let the agent automatically settle negotiable terms that fall within the acceptance ranges you set per term. Anything outside comes to you.</div></div>
...
function OverviewPanel({ st, goTo, kind }) {
  ...
  const TermItems = ({ secId }) => {
    ...
    return fl.map((f) => (
      <div className="os-ov-item" key={f.id}>
        ...
        <span className="os-ov-v">{fmtVal(f, st.values[f.id])}{st.neg[f.id] && f.type !== "date" && <span className="os-ov-neg"><Icon name="triggers" size={9} /> Neg.</span>}</span>
      </div>
    ));
  };
  ...
  const negTag = <span className="os-ov-neg"><Icon name="triggers" size={9} /> Neg.</span>;
  ...
  return (
    <div className="os-ov">
      ...
      <Sec id="pricing" icon="coin" title="Pricing">
      ...
      <Sec id="policies" icon="shield" title="Usage policies">
        {selPol.length ? <div className="os-ov-tags">{selPol.map((k) => <span key={k} className={`pill ${st.policyNeg[k] ? "pill-neg" : "pill-primary"}`}>{POL[k]}{st.policyNeg[k] && <span className="os-ov-neg" style={{ marginLeft: 6 }}><Icon name="triggers" size={9} /> Neg.</span>}</span>)}</div> : <div className="os-ov-empty">No policy selected.</div>}
      </Sec>
      ...
      <Sec id="negotiation" icon="triggers" title="Negotiation & acceptance">
        <div className="os-ov-item"><span className="os-ov-k">Baseline (no negotiation)</span><span className="os-ov-v">{st.baselineMode === "auto" ? "Auto-accept" : "Review first"}</span></div>
        <div className="os-ov-item"><span className="os-ov-k">Contract agent auto-negotiation</span><span className="os-ov-v">{st.autoAgent ? "On" : "Off"}</span></div>
        <div className="os-ov-item"><span className="os-ov-k">Terms</span><span className="os-ov-v">{negTotal} negotiable · {fixedTotal} fixed</span></div>
      </Sec>
    </div>
  );
}
...
- When relevant, explain the difference between Fixed and Negotiable terms, and how the contract agent auto-settles negotiable terms within the provider's ranges.
- Be creative and specific: don't just define a term — tell the provider what does NOT fit THIS offer and profile, and name the exact fields to change (e.g. "99.9% availability is good, except it's Negotiable with no acceptance range — set one on 'Availability / uptime'").
...
- When acceptance ranges are missing, point the provider to define their acceptance baseline in “Negotiation & acceptance”.
...
function legalFindings(cat, st, kind) {
  ...
  if (st.autoAgent) SECTIONS.forEach((s) => {
    const fl = s.fields || (s.groups || []).flatMap((g) => g.fields);
    fl.forEach((f) => { if (neg[f.id] && f.type !== "date" && !acceptSummary(f, acc[f.id])) missing++; });
  });
      ...
      {missingRanges > 0 && (
        <div className="la-baseline-alert">
          ...
          <div className="la-ba-main">
            ...
            <div className="la-ba-d">{missingRanges} negotiable term{missingRanges > 1 ? "s have" : " has"} no range — the contract agent can't auto-settle {missingRanges > 1 ? "them" : "it"} until you set one.</div>
          </div>
...
function laContext(cat, st, kind) {
  ...
  if (section) {
    ...
    fl.forEach((f) => {
      const neg = st.neg[f.id] && f.type !== "date" ? "Negotiable" : "Fixed";
  ...
  ctx += `\nContract-agent auto-negotiation is ${st.autoAgent ? "ON" : "OFF"}. Baseline handling: ${st.baselineMode === "auto" ? "auto-accept" : "review first"}.`;
  const { findings, missingRanges } = legalFindings(cat, st, kind);
  if (findings.length || missingRanges) {
    ...
    if (missingRanges) ctx += `- [RISK] ${missingRanges} negotiable term(s) have no acceptance baseline; the agent can't auto-settle them.\n`;
  }
  return ctx;
}
...
function LegalAssistant({ st, cat, kind, onGoto }) {
  ...
  return (
    <>
      {!open && (
        <button type="button" className="la-fab" onClick={() => setOpen(true)} aria-label="Open legal assistant">
          <Icon name="ai" size={20} /><span>Legal assistant</span>
        </button>
      )}
      <div className={`la-panel ${open ? "open" : ""}`} role="dialog" aria-label="Legal assistant" aria-hidden={!open}>
...
function OfferSettingsApp() {
  ...
  const [tab, setTab] = useState("Offer settings");
  ...
  const HEADS = {
    ...
    pricing: { title: "Pricing", desc: "Your pricing model and business strategy." },
    ...
    negotiation: { title: "Negotiation & acceptance", desc: "How takers engage, and how the contract agent auto-settles negotiable terms." },
  };
  ...
  return (
    <div className="app ui-v2 os-app">
      ...
      <div className="main">
        <header className="topbar cat-topbar">
          <div className="topbar-left"><div className="page-title"><Icon name="offers" size={20} /><h1>My Offers</h1></div></div>
          <div className="topbar-right">
            <button type="button" className="topbar-help hide-mobile"><Icon name="help" size={16} /><span>Help</span></button>
            <button type="button" className="icon-btn ghost notif" aria-label="Notifications"><Icon name="bell" size={18} /><span className="notif-dot">73</span></button>
            <button type="button" className="icon-btn user-btn" aria-label="Account"><Icon name="user" size={18} /></button>
          </div>
        </header>

        <main className="content" id="os-main" tabIndex={-1}>
          <div className="os-page">
            ...
            <div className="os-hero">
              <div className="os-hero-top">
                ...
                <div className="os-hero-actions"><button type="button" className="os-idbtn"><Icon name="key" size={13} /> ID</button><button type="button" className="btn btn-primary" onClick={() => setEditing(true)}><Icon name="edit" size={14} /> Edit</button></div>
              </div>
              <div className="os-cat-row"><span className="os-cat">Job Offers</span></div>
            </div>

            <div className="os-tabbar">
              <div className="os-tabs" role="tablist">{PAGE_TABS.map((t) => <button key={t} type="button" role="tab" aria-selected={tab === t} className={`os-tab ${tab === t ? "active" : ""}`} onClick={() => setTab(t)}>{t}</button>)}</div>
              <div className="os-tabbar-right"><div className="os-progress"><div className="os-progress-bar"><i style={{ width: "100%" }} /></div><span className="os-progress-pct">100%</span></div><button type="button" className="btn btn-primary" style={{ background: "var(--vui-color-secondary)" }}>Unpublish</button></div>
            </div>
            ...
            {tab === "Offer settings" ? (
              <div style={{ marginTop: 16 }}>
                <div className="os-settings-head">
                  ...
                  <p>Everything about this offer, grouped by topic. Pick a category on the left — each term shows its current value at a glance; click a term to change it.</p>
                </div>
            ...
            ) : tab === "Project matchings" ? <MatchingsTab />
              : tab === "Business indicators" ? <BusinessTab />
              : <TechnicalTab />}
          </div>
        </main>
      </div>
      ...
      <LegalAssistant st={st} cat={cat} kind={kind} onGoto={setCat} />
