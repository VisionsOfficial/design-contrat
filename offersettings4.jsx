...
(function () {
  ...
  function fmtVal(field, v) {
    ...
    switch (field.type) {
      ...
      case "selectDeadline":return `${v.a}${v.a === "None" ? "" : " · within " + v.b}`;
  ...
  const MY_TERM_BASELINE = {
    ...
    subcontracting: "Prior written approval",
  ...
  function applyBaseline(d, source, scope) {
    ...
    if (scope === "all") ["pricing", "policies", "personal", "sla", "penalties", "duration", "termination"].forEach(doSection);else
    doSection(scope);
  }
  ...
  function baselinePreview(source, scope) {
    ...
    const SEC_NAMES = { pricing: "Pricing", policies: "Usage policies", personal: "Personal data", sla: "Service levels", penalties: "Commitments & penalties", duration: "Duration & renewal", termination: "Termination", clauses: "Additional clauses" };
  ...
  function seed() {
    ...
    return {
      ...
      resources: ["r_api", "r_hist", "r_verify", "r_score"],
      packages: [
      { _id: "pk1", name: "Starter", sub: "90", billing: "Monthly", setup: "0", api: "0", currency: "EUR", desc: "For pilots and integration tests.", recommended: false, neg: false, accept: {}, note: "",
        resources: ["r_api"], policies: { time_period: true, count: true } },
      { _id: "pk2", name: "Growth", sub: "350", billing: "Monthly", setup: "250", api: "0", currency: "EUR", desc: "Production usage, one integration.", recommended: true, neg: true, accept: { min: 300, max: 350 }, note: "",
        resources: ["r_api", "r_hist", "r_verify"], policies: null },
      { _id: "pk3", name: "Scale", sub: "1400", billing: "Monthly", setup: "500", api: "0", currency: "EUR", desc: "High volume, multi-service integration.", recommended: false, neg: true, accept: { min: 1200, max: 1400 }, note: "",
        resources: ["r_api", "r_hist", "r_verify", "r_score"], policies: { no_restriction: true, notification: true } }],
  ...
  function ValueControl({ field, value, onChange }) {
    switch (field.type) {
      ...
      case "selectDeadline":return <><Sel value={value?.a} onChange={(a) => onChange({ ...value, a })} options={field.options} />{value?.a !== "None" && <><span className="os-unit">within</span><Sel value={value?.b} onChange={(b) => onChange({ ...value, b })} options={field.deadlines} /></>}</>;
  ...
  function AcceptControl({ field, accept, onChange }) {
    switch (field.type) {
      ...
      case "select":case "multiselect":case "yesno":case "twoSelect":case "selectDeadline":{
  ...
  function AiFillBar({ section, st, set }) {
    ...
    const run = async () => {
      ...
      } catch (e) {setErr("Couldn’t auto-complete just now. Please try again.");} finally
      {setBusy(false);}
    };
  ...
  // ─── RESOURCE DRAWER (slide-over holding the resource form) ───────────────────
  function ResourceDrawer({ res, onClose }) {
    const { ResourceForm, RF_EMPTY, RF_TABS } = window.VTResourceForm;
    const [tab, setTab] = useState("general");
    const [v, setV] = useState(() => RF_EMPTY(res.kind, res.name));
    ...
    const idx = RF_TABS.findIndex((t) => t.id === tab);
    const last = idx === RF_TABS.length - 1;
    return (
      <>
      <div className="rd-scrim" onClick={onClose} />
      <aside className="rd" role="dialog" aria-modal="true" aria-label={res.name ? `Edit ${res.name}` : "Add resource"}>
        <div className="rd-head">
          <div><h2>{res.name ? "Edit resource" : "Add resource"}</h2><p>Please provide here general information on the resource that will be displayed in the catalogue. This is to help potential buyers to identify your resources added value and unique features!</p></div>
          <button type="button" className="rd-x" onClick={onClose} aria-label="Close"><Icon name="x" size={20} /></button>
        </div>
        <div className="rd-body"><ResourceForm kind={res.kind} value={v} onChange={(k, x) => setV((p) => ({ ...p, [k]: x }))} tab={tab} onTab={setTab} /></div>
        <div className="rd-foot">
          <button type="button" className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <span className="sb-txt rd-spacer">{saved ? <b>✓ Resource saved</b> : `Step ${idx + 1} of ${RF_TABS.length}`}</span>
          {last ?
            <button type="button" className="btn btn-primary" onClick={() => {setSaved(true);setTimeout(onClose, 700);}}><Icon name="check" size={14} /> Save</button> :
            <button type="button" className="btn btn-primary" onClick={() => setTab(RF_TABS[idx + 1].id)}>Next</button>}
        </div>
      </aside>
    </>);
  ...
  // ─── OFFER CONTENT ────────────────────────────────────────────────────────────
  function ContentPanel({ kind, pdOn, st, set, resMode = "Slide-over" }) {
    const RES = window.OS4Packages.RESOURCES;
    const ids = st.resources || [];
    const [adding, setAdding] = useState(false);
    const [drawer, setDrawer] = useState(null);
    const openRes = (r) => {if (resMode === "Dedicated page") {location.href = `FINAL Resource Edit.html?kind=${encodeURIComponent(r.kind || "Data")}&name=${encodeURIComponent(r.name || "")}&back=${encodeURIComponent(location.pathname.split("/").pop())}`;} else {setDrawer({ kind: r.kind || "Data", name: r.name || "" });}};
    const usedIn = (id) => (st.packages || []).filter((pk) => window.OS4Packages.pkRes(st, pk).includes(id)).length;
    const rest = RES.filter((r) => !ids.includes(r.id));
    return (
      <>
      <p style={{ margin: "10px 0 14px", fontSize: 13, color: "var(--text-muted)" }}>Detail exactly what your offer comprises. Packages draw from this list — each one can include all of it, or only part of it.</p>
      {ids.map((id) => {const r = RES.find((x) => x.id === id) || {};const n = usedIn(id);return (
            <div className="os-content-row" key={id}>
          {r.kind === "Service" ?
              <div className="os-content-ic" style={{ background: "var(--vui-color-secondary)" }}><Icon name="triggers" size={20} /></div> :
              <div className="os-content-ic">0101<br />0110</div>}
          <div className="os-content-main">
            <div className="os-content-kind">{r.kind}{pdOn && r.kind === "Data" && <span className="os-pd-tag"><Icon name="lock" size={9} /> Personal data</span>}</div>
            <div className="os-content-name">{r.name}</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{r.meta} · in {n} of {(st.packages || []).length} packages</div>
          </div>
          <div className="os-content-actions"><button type="button" className="os-mini-btn" onClick={() => openRes(r)}>{resMode === "Dedicated page" ? <><Icon name="chevronRight" size={13} /> Open</> : <><Icon name="edit" size={13} /> Edit</>}</button><button type="button" className="os-mini-btn danger" onClick={() => set((x) => {x.resources = x.resources.filter((y) => y !== id);x.packages.forEach((pk) => {pk.resources = (pk.resources || []).filter((y) => y !== id);});})}><Icon name="trash" size={13} /> Remove</button></div>
        </div>);
        })}
      <div className="os-add-row">
        <button type="button" className="os-add-btn" onClick={() => openRes({ kind: "Data" })}><Icon name="plus" size={14} /> Add new Data Resource</button>
        <button type="button" className="os-add-btn" onClick={() => openRes({ kind: "Service" })}><Icon name="plus" size={14} /> Add new Service Resource</button>
        <button type="button" className="os-add-btn ghost" onClick={() => setAdding((a) => !a)}><Icon name="search" size={14} /> Browse existing resource</button>
      </div>
      {adding && (rest.length ?
        <div className="pkx-res" style={{ marginTop: 10 }}>{rest.map((r) =>
          <button key={r.id} type="button" className="pkx-rchip" onClick={() => {set((x) => {x.resources = [...x.resources, r.id];});setAdding(false);}}>
              <span className={`pkx-rk ${r.kind === "Service" ? "svc" : "data"}`}>{r.kind}</span><span className="pkx-rn">{r.name}</span><span className="pkx-rm">{r.meta}</span><Icon name="plus" size={13} />
            </button>)}</div> :
        <div className="pk-miss" style={{ marginTop: 10 }}><Icon name="info" size={12} /> Every resource you own is already in this offer.</div>)}
      {drawer && <ResourceDrawer res={drawer} onClose={() => setDrawer(null)} />}
    </>);
  ...
  const pkIssues = (pk) => {const m = [];if (String(pk.sub == null ? "" : pk.sub).trim() === "") m.push("subscription pricing");if (!pk.billing) m.push("billing period");return m;};
  const EMPTY_PKG = (s) => ({ _id: pkUid(), name: "", sub: "", billing: "Monthly", setup: "0", api: "0", currency: s && s.pricing && s.pricing.currency || "EUR", desc: "", recommended: false, neg: false, accept: {}, note: "", resources: s ? [...(s.resources || [])] : null, policies: null });
  // A package "carries its own policies" when its effective set differs from the offer's.
  const pkOwnPol = (st, pk) => window.OS4Packages.pkDiff(st, pk).secs.includes("policies");
  ...
  const pkgPolicies = (st, pk) => window.OS4Packages.pkPol(st, pk);
  const pkgPolLabels = (st, pk) => PKG_POL.filter((x) => pkgPolicies(st, pk)[x.id]).map((x) => x.t);

  const PackagesSection = (props) => React.createElement(window.OS4Packages.PackagesSection, props);

  const PRICING_MODES = [
  { id: "single", t: "Single price", d: "One published price that applies to every taker." },
  { id: "packages", t: "Packages", d: "Several variants of the same offer — own price, own resources, own usage policies." }];


  function PricingPanel({ st, set, goTo }) {
    const p = st.pricing;
    const upd = (k, v) => set((s) => {s.pricing[k] = v;});
    const mode = st.pricingMode || "single";
    const negOn = !!st.pricingNeg.sub;
    ...
    return (
      ...
      {mode === "single" ?
        <>
          <div className="os-note"><Icon name="info" size={15} /> <span>If pricing is not applicable, set the numeric fields to <b>0</b>. A price marked <b>negotiable</b> can be moved by the contract agent inside the range you define — everything else is fixed once published.</span></div>
          ...
          <div className="pkx-negrow" style={{ marginTop: 14 }}>
            <button type="button" className={`osf-negbtn${negOn ? " on" : ""}`} aria-pressed={negOn} onClick={() => set((s) => {s.pricingNeg.sub = !negOn;})}><span className="mini-toggle"><i /></span>Price is negotiable</button>
            {negOn && <MiniNegStrip autoAgent={st.autoAgent} note={st.pricingNotes.sub || ""} onNote={(v) => set((s) => {s.pricingNotes.sub = v;})} acceptEl={<span className="os-range"><Num value={(st.pricingAccept.sub || {}).min} onChange={(min) => set((s) => {s.pricingAccept.sub = { ...(s.pricingAccept.sub || {}), min };})} /><span className="dash">to</span><Num value={(st.pricingAccept.sub || {}).max} onChange={(max) => set((s) => {s.pricingAccept.sub = { ...(s.pricingAccept.sub || {}), max };})} /><span className="os-unit">{p.currency}</span></span>} />}
          </div>
        </> :

        <PackagesSection st={st} set={set} />
        }
    </>);
  ...
  function PoliciesPanel({ st, set }) {
    ...
    return (
      ...
      {st.pricingMode === "packages" && (() => {const own = (st.packages || []).filter((pk) => pkOwnPol(st, pk));return (
            <div className="os-note"><Icon name="coin" size={15} /> <span>These are the <b>offer-level</b> policies — every package starts from them and can be adjusted in <b>Pricing &amp; Packages</b>. {own.length ? <>Currently <b>{own.length} package{own.length > 1 ? "s" : ""}</b> differ{own.length > 1 ? "" : "s"}: {own.map((pk) => pk.name || "Untitled").join(", ")}.</> : <>No package differs from them today.</>}</span></div>);
        })()}
  ...
  function OverviewPanel({ st, goTo, kind }) {
    ...
    return (
      ...
      <Sec id="content" icon="database" title="Offer content">
        {(st.resources || []).map((id) => {const r = window.OS4Packages.resById(id) || {};return (
              <div className="os-ov-item" key={id}><span className="os-ov-k">{r.name}</span><span className="os-ov-v">{r.kind} · {r.meta}</span></div>);
          })}
      </Sec>

      <Sec id="pricing" icon="coin" title="Pricing & Packages" badges={st.pricingMode === "packages" ? <span className="pill pill-primary">{st.packages.length} packages</span> : null}>
        {st.pricingMode === "packages" ?
          ...
          <div className="os-ov-item" key={pk._id}>
                ...
                <span className="os-ov-v">{(() => {const c = pk.currency || p.currency;return <>{String(pk.sub == null ? "" : pk.sub).trim() === "" ? "—" : pkFmt(pk.sub) + " " + c} · {pk.billing || "Monthly"}{pkNum(pk.setup) ? " · " + pkFmt(pk.setup) + " " + c + " set-up" : ""}</>;})()}{pk.neg && negTag}</span>
              </div>
          ) :
          <>
      ...
      <Sec id="policies" icon="shield" title="Usage policies">
        ...
        {st.pricingMode === "packages" && st.packages.filter((pk) => pkOwnPol(st, pk)).map((pk) =>
  ...
  // ─── OFFER INFORMATION (identity fields, ex-"Edit offer" modal) ───────────────
  function OfferInfoPanel() {
    const [cats, setCats] = useState(["Hobbies"]);
    return (
      <div className="oi-form">
      <label className="field"><span className="field-label">Offer name<em>*</em></span><input className="input" defaultValue="anthony_hobbies" maxLength={22} /><span className="field-hint">22 characters maximum</span></label>
      <label className="field"><span className="field-label">Offer caption<em>*</em></span><textarea className="textarea oi-caption" defaultValue="description hobbies" maxLength={69} /><span className="field-hint">69 characters maximum</span></label>
      <div className="field">
        <span className="field-label">Detailed offer description<em>*</em></span>
        <div className="oi-rte">
          <div className="oi-rte-bar">
            <span className="oi-rte-sel">Normal<Icon name="chevronDown" size={12} /></span>
            <span className="oi-rte-sep" />
            <button type="button" className="oi-rte-b oi-rte-u" title="Text colour">A</button>
            <button type="button" className="oi-rte-b" style={{ fontWeight: 800 }} title="Bold">B</button>
            <button type="button" className="oi-rte-b" style={{ fontStyle: "italic" }} title="Italic">I</button>
            <button type="button" className="oi-rte-b oi-rte-u" title="Underline">U</button>
            <button type="button" className="oi-rte-b" style={{ textDecoration: "line-through" }} title="Strikethrough">S</button>
            <span className="oi-rte-sep" />
            <button type="button" className="oi-rte-b" title="Align">≡</button>
            <button type="button" className="oi-rte-b" title="Numbered list">1.</button>
            <button type="button" className="oi-rte-b" title="Bullet list">•</button>
            <span className="oi-rte-sep" />
            <button type="button" className="oi-rte-b" title="Link"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M10 13a5 5 0 0 0 7 0l2-2a5 5 0 0 0-7-7l-1 1" /><path d="M14 11a5 5 0 0 0-7 0l-2 2a5 5 0 0 0 7 7l1-1" /></svg></button>
            <button type="button" className="oi-rte-b" title="Clear formatting">Tx</button>
          </div>
          <textarea className="oi-rte-area" defaultValue="detailed description hobbies" />
        </div>
        <span className="field-hint">50 000 characters maximum</span>
      </div>
      <div className="grid-2">
        <div className="field">
          <span className="field-label">Category<em>*</em></span>
          <div className="select-wrap">
            <select className="input select" value="" onChange={(e) => {const v = e.target.value;if (v && !cats.includes(v)) setCats((c) => [...c, v]);}}>
              <option value="">Browse</option><option>Hobbies</option><option>Job Offers</option><option>Skills</option><option>Training</option>
            </select>
            <Icon name="chevronDown" size={14} className="select-chev" />
          </div>
          {cats.length > 0 && <div className="oi-chips">{cats.map((c) => <span key={c} className="oi-chip">{c}<button type="button" onClick={() => setCats((x) => x.filter((y) => y !== c))} aria-label={`Remove ${c}`}><Icon name="x" size={10} /></button></span>)}</div>}
        </div>
        <label className="field"><span className="field-label">Country or region<em>*</em></span><div className="select-wrap"><select className="input select" defaultValue="Europe"><option>Europe</option><option>France</option><option>Belgium</option><option>Germany</option></select><Icon name="chevronDown" size={14} className="select-chev" /></div></label>
      </div>
      <div className="field">
        <span className="field-label">Offer picture</span>
        <div className="oi-drop" role="button" tabIndex={0}><Icon name="upload" size={20} /><span className="oi-drop-t">Drop your image or browse</span><span className="oi-drop-s">JPEG, PNG, JPG, SVG · 280×120</span></div>
      </div>
      <div className="field">
        <span className="field-label">Documents</span>
        <div className="oi-drop" role="button" tabIndex={0}><Icon name="upload" size={20} /><span className="oi-drop-t">Drag and drop files here, or browse</span><span className="oi-drop-s">PDF, images (JPG, PNG, GIF)</span></div>
      </div>
    </div>);
...
- You are not a lawyer: add a brief one-line reminder to have consequential commitments reviewed by legal counsel — only when giving weighty advice, not on every reply.`;
  ...
  function legalFindings(cat, st, kind) {
    ...
    if (cat === "pricing" && st.pricingMode === "packages") {
      ...
      if (bad.length) add("warn", `${bad.length} package${bad.length > 1 ? "s are" : " is"} incomplete`, `A subscription price and a billing period are mandatory on every package — an incomplete formula cannot be published or written into a contract.`, bad.map((pk) => pk.name || "Untitled package"));
      if (!bad.length && pkgs.length > 1) add("ok", `${pkgs.length} packages are consistent`, `Each formula states its price, its billing period, the resources it includes and the usage policies that apply — takers can compare and pick without asking you.`);
    }
    ...
    if (section && section.id === "sla") {
      ...
      if (av) {
        ...
        if (backed) add("ok", `Availability ${av} is a strong, credible commitment`, `It reads well to takers and a penalty rule already backs it — nothing to change here.`, ["Availability / uptime"]);else
        add("warn", `${av} availability is fine — except nothing backs it`, `You promise ${av} uptime, yet no penalty rule covers it. Without a service credit or fee waiver a taker has no recourse if you miss it, which weakens the clause.`, ["Availability / uptime", "Commitments & penalties"], "penalties");
      }
    ...
    return (
      ...
      {missingRanges > 0 &&
        <div className="la-baseline-alert">
          ...
          <button type="button" className="la-ba-btn" onClick={() => onGoto("duration")}>Define baseline</button>
        </div>
        }
  ...
  function OfferSettingsApp() {
    const [t, setTweak] = useTweaks({ offerKind: "Data", resEdit: "Slide-over" });
    ...
    const HEADS = {
      ...
      info: { title: "Offer information", desc: "Identity of the offer as shown in the catalogue." },
      content: { title: "Offer content", desc: "The products, services or data your offer comprises." },
      pricing: { title: "Pricing & Packages", desc: "Your pricing model — a single price, or several packages, each a variant of the same offer with its own price, resources and usage policies." },
    ...
    return (
      <AppLayout title={<span className="os-crumb-title"><a href="My Offers.html">My offers</a><span className="sep">/</span><b>check_free_offer</b></span>} activeId="offers-all" className="os-app">
          <div className="os-page">
            ...
            <div className="os-hero">
              <div className="os-hero-top">
                ...
                <div className="os-hero-actions"><button type="button" className="os-idbtn"><Icon name="key" size={13} /> ID</button><button type="button" className="os-idbtn" onClick={() => {try {navigator.clipboard.writeText(location.href);} catch (e) {}}}><Icon name="share" size={13} /> Copy link</button><div className="os-progress"><div className="os-progress-bar"><i style={{ width: "100%" }} /></div><span className="os-progress-pct">100%</span></div><button type="button" className="btn btn-primary" style={{ background: "var(--vui-color-secondary)" }}>Unpublish</button></div>
              </div>
                ...
                <React.Fragment key={g.group}>
                    ...
                    return (
                      <button key={c.id} type="button" className={`os-rail-item ${cat === c.id ? "active" : ""}`} onClick={() => setCat(c.id)} aria-current={cat === c.id ? "true" : undefined}>
                                ...
                                {c.id === "pricing" && st.pricingMode === "packages" && <span className="os-rail-badge" title={`${st.packages.length} packages`}>{st.packages.length}</span>}
                    ...
                    <section className="os-panel">
                      ...
                      <div className="os-panel-body">
                        ...
                        {cat === "info" && <OfferInfoPanel />}
                        {cat === "content" && <ContentPanel kind={kind} pdOn={pd.enabled} st={st} set={set} resMode={t.resEdit} />}
                        {cat === "pricing" && <PricingPanel st={st} set={set} goTo={setCat} />}
      ...
      <TweaksPanel title="Offer type">
        ...
        <TweakSection label="Resource editing" />
        <TweakRadio label="Edit a resource via" value={t.resEdit} options={["Slide-over", "Dedicated page"]} onChange={(v) => setTweak("resEdit", v)} />
      </TweaksPanel>
    </AppLayout>);

  }

  window.OS4 = { clone, isEmpty, eq, fmtVal, Sel, Num, ValueControl, MiniNegStrip, PKG_POL, RETENTIONS: PD.retention, pkUid, pkNum, pkFmt, pkIssues, EMPTY_PKG, pkgPolicies, pkgPolLabels, pkOwnPol, canNeg };
