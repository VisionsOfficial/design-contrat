// VisionsTrust — Basket · variante "étape 2 par offre" (ancienne version de l'étape 2).
// Identique à basket3.jsx, sauf l'étape 2 : une carte par offre, réglée offre par offre.
...
const lsKey = () => "vt.basket.v6b." + SCENARIO;
...
function OfferFacts({ offer, pkgId }) {
  ...
  return (
    <div className="bk-facts">
      <div className="bk-fact">
        ...
        <span className="bk-fact-v">{main}</span>
        ...
        {pkgs.map((pk) => (
          <div className={`bk-pkg-opt${pk.recommended ? " reco" : ""}${chosenId === pk.id ? " on" : ""}`} key={pk.id}>
            ...
            <ul className="bk-pkg-list">
              ...
              <li><Icon name="lock" size={12} /> Fixed price</li>
            </ul>
...
// ─── STEP 1 · OFFER CARD ─────────────────────────────────────────────────────────
function OfferCard({ offer, terms, proposal, decision, pkgId, help, phase = "negotiate", onChoosePackage, onConcede, onSetField, onProposeField, onAcceptAll, onCounterAll, onReopen, onEdit, onView, onSave, onRemove }) {
  ...
  return (
    <article className={`bk2-card ${stateClass}`}>
      ...
      <OfferFacts offer={offer} pkgId={pkgId} />
      {hasPkgs && <PackagePicker offer={offer} chosenId={pkgId} compact={!needsPkg} onChoose={onChoosePackage} />}
      {/* ── SETTLED banner ─────────────────────────── */}
      {needsPkg ? null : reviewing ? (() => {
        const VERDICT = { accepted: "Provider's baseline accepted as published.", countered: d.withBase.length === 0 ? "Counter-offer ready — no baseline covered this offer." : "Counter-offer ready — your baseline applied.", edited: "Counter-offer ready — your own values." };
        const neutral = d.settled ? false : (!hasBaseline() || d.gapCount > 0 || d.negs.length === 0);
        const icon = d.settled ? "check" : !hasBaseline() ? "info" : d.gapCount > 0 ? "triggers" : d.negs.length === 0 ? "info" : "check";
        const txt = d.settled ? VERDICT[d.mode]
          : !hasBaseline() ? "No acceptance baseline to check against — read the fields in step 2, or take the offer as published."
          : d.gapCount > 0 ? `${d.gapCount} field${d.gapCount !== 1 ? "s" : ""} to settle in step 2 — adjust the baseline there, or take the offer as published.`
          : d.negs.length === 0 ? "Nothing to negotiate on this offer — every field is fixed as published."
          : "Nothing sits below your baseline — step 2 is a formality on this offer.";
        return <div className={`bk2-allok${neutral ? " neutral" : ""}`}><Icon name={icon} size={14} /> {txt}</div>;
      })() : d.settled ? (
...
function PricingCell({ offer, pricing, pkgId }) {
  const pk = pkgById(offer, pkgId);
  if (!pk) return <div className="bk-conf-v">{fmtPrice(pricing)}</div>;
  const cur = offer.pricing.currency;
  return (
...
function BasketApp({ help = "tour" }) {
  const seed = () => {
    const proposals = {}; const decisions = {}; const packageBy = {};
    ITEMS.forEach((o) => {
      const p = {}; termsFor(o, null).negFields.forEach((f) => { p[f.id] = clone(f.baseline); });
      proposals[o.id] = p;
    ...
    return {
      ...
      proposals, pricing, decisions, packageBy,
    };
  };
  ...
  const load = () => {
    try { const raw = localStorage.getItem(lsKey()); if (raw) { const s = seed(); const j = JSON.parse(raw); return { ...s, ...j, proposals: mergeProposals(s.proposals, j.proposals), pricing: { ...s.pricing, ...j.pricing }, decisions: { ...s.decisions, ...j.decisions }, packageBy: { ...s.packageBy, ...(j.packageBy || {}) } }; } } catch (e) {}
    return seed();
  };
  ...
  const termsIn = (s2, id) => termsFor(byId(id), s2.packageBy[id]);
  // Picking (or changing) a package re-derives the terms, so the negotiation restarts.
  const choosePackage = (offerId, pkgId) => setSt((s2) => {
    const p = {}; termsFor(byId(offerId), pkgId).negFields.forEach((f) => { p[f.id] = clone(f.baseline); });
    return { ...s2, packageBy: { ...s2.packageBy, [offerId]: pkgId }, proposals: { ...s2.proposals, [offerId]: p }, decisions: { ...s2.decisions, [offerId]: { mode: "pending", conceded: [] } } };
  });
  ...
  const resetField = (offerId, fieldId) => setField(offerId, fieldId, clone(termsOf(byId(offerId)).negFields.find((f) => f.id === fieldId).baseline));
  ...
  const acceptAll = (offerId) => setSt((s) => {
    const p = {}; termsIn(s, offerId).negFields.forEach((f) => { p[f.id] = clone(f.baseline); });
  ...
  const counterAll = (offerId) => setSt((s) => {
    ...
    const p = { ...cur };
    termsIn(s, offerId).negFields.forEach((f) => { if (f.userBase) p[f.id] = clone(baselineTarget(f, f.userBase, cur[f.id])); });
  ...
  const rows = selectedOffers.map((o) => {
    const d = deriveOffer(termsOf(o), st.proposals[o.id] || {}, st.decisions[o.id] || { mode: "pending", conceded: [] });
  ...
  return (
    <AppLayout title="Basket" activeId="offers" className="bk-app">
          ...
          <div className="bk-page">
            {empty ? (
              ...
              <div className="bk-flow">
                ...
                {step === 1 && (
                  <div className="bk-stepbody">
                    ...
                    {selectedOffers.length === 0 ? (
                      ...
                      <OfferCard key={o.id} offer={o} terms={termsOf(o)} proposal={st.proposals[o.id]} decision={st.decisions[o.id] || { mode: "pending", conceded: [] }}
                ...
                {step === 2 && (
                  <div className="bk-stepbody">
                    <div className="bk-step-intro"><h2>Adjust the baseline</h2><p>Each offer is checked against your acceptance baseline. Settle every one — take the provider's baseline, counter with your own, or edit field by field.</p>{help !== "none" && <button type="button" className="bk-guide-btn" onClick={() => (help === "explain" ? setDockOpen((o) => !o) : guide.start())}><Icon name="help" size={14} /> {help === "explain" ? (dockOpen ? "Hide the explainer" : "How this works") : guide.seen ? "Replay the guide" : "How this works"}</button>}</div>
                    <BaselineRecall />
                    {selectedOffers.map((o) => (
                      <OfferCard key={o.id} offer={o} terms={termsOf(o)} proposal={st.proposals[o.id]} decision={st.decisions[o.id] || { mode: "pending", conceded: [] }}
                        pkgId={st.packageBy[o.id]} help={help} onChoosePackage={(pid) => choosePackage(o.id, pid)}
                        onConcede={(fid) => concedeGap(o.id, fid)} onSetField={(fid, v) => setField(o.id, fid, v)} onProposeField={(fid, v) => proposeField(o.id, fid, v)} onAcceptAll={() => acceptAll(o.id)} onCounterAll={() => counterAll(o.id)}
                        onReopen={() => reopen(o.id)} onEdit={() => openDrawer(o.id, "edit")} onView={() => openDrawer(o.id, "review")}
                        onSave={() => moveToSaved(o.id)} onRemove={() => removeFromSelected(o.id)} />
                    ))}
                    <div className="bk-nav">
                      ...
                      <button type="button" className="bk-confirm" disabled={pendingCount > 0} onClick={() => goTo(3)}>
                ...
                {step === 4 && (
                  <div className="bk-stepbody">
                    ...
                    <ConfirmStep rows={rows} pricing={st.pricing} proposals={st.proposals} packageBy={st.packageBy} target={target} targetCaption={targetCaption} onView={(id) => openDrawer(id, "review")} />
                    <div className="bk-nav">
      ...
      {drawerOffer && (
        <TermsDrawer offer={drawerOffer} terms={termsOf(drawerOffer)} proposal={st.proposals[drawerOffer.id]} decision={st.decisions[drawerOffer.id] || { mode: "pending", conceded: [] }}
          open={drawerOpen} initialMode={drawer.mode} readOnly={step !== 2} onClose={closeDrawer}
